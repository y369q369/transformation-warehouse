...
parser.add_argument('--startDate', type=str, help='开始日期', default='2023-03-01')
parser.add_argument('--endDate', type=str, help='结束日期', default='2023-04-12')
...
def get_actual(date_str):
    ...
    actual_query = 'import "date"\n' \
                   ...
                   '  |> drop(columns: ["_start", "_stop", "_field", "_measurement", "platform_id"])' \
...
def get_forecast(date_str):
    ...
    forecast_sql = 'SELECT \n' \
                   ...
                   'FROM \n' \
                   '    pv_device_per_grid_full a, gfyc_shortforecast_grid b \n' \
                   'WHERE \n' \
                   '    a.grid_id = b.grid_id \n' \
            ...
            if child_id in device_power_data:
                ...
                for time_str in device_power_data[child_id]:
                    if time_str not in platform_power_data[self]:
                        ...
                        platform_power_data[self][time_str]['forecast'] = \
                            device_power_data[child_id][time_str]['forecast']
                    else:
                        platform_power_data[self][time_str]['forecast'] += \
                            device_power_data[child_id][time_str]['forecast']
...
# 表头
table_header = ['编号', '名称', '开机容量', '计算点数', '准确率（%）', '是否异常(0:正常，1:异常)']
# 表空间
sheet_list = ['台区']
...
if __name__ == '__main__':
    ...
    if args.startDate is None or args.endDate is None:
        # date_list.append('2023-03-01')
        # date_list.append('2023-03-02')
