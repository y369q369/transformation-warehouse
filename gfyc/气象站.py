...
def load_forecast(day):
 forecast = pd.read_sql(
  f"select forecast,time from gfyc_shortforecast where  id =3241106 and time between '{day} 00:00:00' and '{day} 23:59:00' ",
...
if __name__ == '__main__':
 for day in daybetween:
  ...
  for j in tqdm(range(len(ratio))):
   ...
   rmses.append(rmse1)
  pd.DataFrame({'dev_nos': dev_nos, 'length': length, 'rmses': rmses}).to_csv(f'qx_{day}_{county_code}.csv')
