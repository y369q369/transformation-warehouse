...
def catalog_list(url):
    ...
    author = selector.css('#info p:nth-child(2) a::text').get()
    chapter_list = []

    detail_list = selector.css('.mulu_list li')[1:]
...
def download(info):
    ...
    for thread_content_list in content_list:
        for chapter_content in thread_content_list:
            ...
            context_list = selector.css('#htmlContent::text').getall()
            for content in context_list:
                detail += content.replace('\xa0', '') + '\n\n'
                # detail += content + '\n\n'

            # 去除广告内容
            filter_str = '一秒记住【奇书网 www.qishu.org】，精彩小说无弹窗免费阅读！'
...
if __name__ == '__main__':
    url = 'https://www.qishu.org/article/75/'
